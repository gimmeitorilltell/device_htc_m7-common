 ...
 * Copyright (C) 2018 CandyRoms Project
 *
...
#define LOG_TAG "MSM8960PowerHAL"
#include <utils/Log.h>
...
static int boostpulse_open(struct msm8960_power_module *msm8960)
{
    char buf[80];
    int len;
    static int gpu_boost_fd = -1;

    pthread_mutex_lock(&msm8960->lock);

    if (msm8960->boostpulse_fd < 0) {
        msm8960->boostpulse_fd = open(BOOSTPULSE_PATH, O_WRONLY);

        if (msm8960->boostpulse_fd < 0) {
            if (!msm8960->boostpulse_warned) {
                strerror_r(errno, buf, sizeof(buf));
                ALOGE("Error opening %s: %s\n", BOOSTPULSE_PATH, buf);
                msm8960->boostpulse_warned = 1;
            }
        }
    }
    {
    ...
    pthread_mutex_unlock(&msm8960->lock);
    return msm8960->boostpulse_fd;
}

static void set_feature(struct power_module *module, feature_t feature, int state)
{
    struct msm8960_power_module *msm8960 =
            (struct msm8960_power_module *) module;
    switch (feature) {
...
static void msm8960_power_hint(struct power_module *module, power_hint_t hint,
                                void *data)
{
    struct msm8960_power_module *msm8960 =
            (struct msm8960_power_module *) module;
    ...
    switch (hint) {
     case POWER_HINT_INTERACTION:
        if (boostpulse_open(msm8960) >= 0) {
            len = write(msm8960->boostpulse_fd, "1", 1);
...
static int msm8960_power_open(const hw_module_t* module, const char* name,
                    hw_device_t** device)
{
    ...
    if (strcmp(name, POWER_HARDWARE_MODULE_ID) == 0) {
        ...
        if (dev) {
            ...
            dev->powerHint = msm8960_power_hint;
...
static struct hw_module_methods_t power_module_methods = {
    .open = msm8960_power_open,
};

struct msm8960_power_module HAL_MODULE_INFO_SYM = {
    base: {
        common: {
